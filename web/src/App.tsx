...
export default function App() {
  ...
  async function handleSync() {
    ...
    const serverTime = await syncWithServer(() => dirty, async (serverNotes) => {
  ...
  return (
    <div className="flex min-h-screen flex-col">
      <header className="flex items-center justify-between border-b border-gray-200 px-4 py-3">
        <h1 className="m-0 text-lg font-semibold">Offline-First Notes (Web)</h1>
      ...
      <main className="grid flex-1 gap-4 p-4 md:grid-cols-[320px_1fr]">
      ...
      <footer className="border-t border-red-400 px-4 py-3 text-xs text-gray-500">
        TypeScript + Vite • IndexedDB fallback to localStorage • Debounced search • Custom store
      </footer>
    </div>
  );
}
